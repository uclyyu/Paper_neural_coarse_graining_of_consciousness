...
\rlstart{3}
        In this section, we formally introduce our theory \tnamefull \todo{We need a good name}.
        
        We starts our argument from a presumptive fact that every human conscious precept is ex trimly informative. Our conscious experience always consists of high multidimensional information involved multiple modalities. Considering information as resolved uncertainty, every conscious percept rules out enormous other possible conscious states in the state space, therefore, gives huge amount of of information. This notion is similar to one of the axiom, \textit{information}, in Integrated Information Theory (IIT 3.0, \cite{oizumi2014phenomenology}) which claims that \myQuote{...an experience of pure darkness is what it is by differing, in its particular way, from an immense number of other possible experiences...} \citep[p. 2]{oizumi2014phenomenology}.
        
        Based on this presumptive fact, we further take a radical standpoint and postulate that consciousness fundamentally associate with information in the physical reality (information correlates of consciousness, \cite{chalmers1996conscious, tononi2004information, gamez2011information}) rather than neuronal states (neural correlates of consciousness). That is, consciousness should be information-based. The amount of information comprising a conscious percept should be equivalent to information we can obtain from a specific physical event.  
        
        Nevertheless, even though information involved in every conscious percept is extremely rich, it is still bounded, i.e. boundary exists. Presuming consciousness is information-based, we can deal with the boundary problem \cite{goff2006experiences} by information closure. Additionally, our conscious experience contains not merely trivial informational rather it always provides rich information about the environmental and our bodily states. Therefore, we argue that NTIC is the core entity \highlightAndComment{responsible}{Help me to think of a better term} for consciousness.
...
\rlstart{3}
		\subsection{Measure of consciousness/information correlate of consciousness}	
			...
			\begin{ants}
			@ In this thesis, we postulate that NTIC is the core entity of information correlate of consciousness.
			
			@ We further propose properties of consciousness correlate properties of NTIC
			
			
			quantitative hypotheses of conscious level and conscious content. 
			
			@ We first hypothesise that, at time $t$, levels of consciousness $C_{t}^{Level}$ corresponds to the degree of NTIC at coarse-grained levels $NTIC_{t}$.
			...
			\subsubsection{Level of Consciousness correlates degrees of NTIC of a process}
    			\begin{ants}
    			@ Several key insights about the conscious level can be derived from our theory.
    			
    			@ Equation \ref{eq:nticObjective} implies that conscious level positive correlates the complexity of the environment that the NTIC process encoded. 
    			
    			@ This implies that an agent who forms an NTIC process to lives and adapt to a more complex environment should have a higher conscious level than another agent who lives in an environment with more simple dynamics. 
    			
    			@ Channel capacity between the current state and the future state of the NTIC process also determines the level of consciousness.
    			
    			@ \todo{Need an example here} A small creatures may live in a simple environment and forms a NTIC process. The predictive information for the next state of the process may be very low. It has consciousness but the levels of consciousness is low. 
    			
    			@ Another implication about levels of consciousness from Equation \ref{eq:nticObjective} predictability of the system future state given the current state is crucial for the conscious level. This indicates that the if the system dynamics are highly scholastic and much unpredictable, the conscious level should be low. This notion explicitly indicates that information processed at neuronal scales which encounter heavy noise and are highly scholastic cannot form high level of consciousness.
    			
    			@ Similarly, at macro-scales, if coarse-graining causes coarse-grained macro-variable lose predictive fine-grain information, the predictability become low and also the channel capacity between the current state and future state becomes very small, NTIC would be low as well. K
    			
    			@ \todo{modify this figure}
    			
    				\begin{figure}[H]				\includegraphics[width=\textwidth]{WritingMaterials/FoxitReader_2019-01-31_19-03-59.png}
    			...
    			@ This non-monotonic relationship bwetween the level of coarse-graining in the neural system and the degree of NTIC explains why only information processed at a certain level can become conscious contents.  
    			
    			
    			\end{ants}

    		\subsubsection{Coarse-graining states correlates conscious contents}
    			\begin{ants}
				@ We claim that conscious contents correspond to the state of the NTIC process.
				
				@ This implies that the complexity of conscious contents is determined by the amount of information encompassed in the NTIC process.
				
				@ Therefore, larger state space of the NTIC process has, richer  experience the process can be conscious of (e.g., a NTIC process with high dimensionality).
				
				@ Crucially, even experience of darkness is rich because it rules out other enormous potential state that the NTIC process can be in. Equivalently, knowing the state of the NTIC process when the process experiences darkness also mean acquisition of information of the system.\todo{bad wording}
				
    			@ This is similar to the the \textit{Information} axiom in integrated information theory which claims that \myQuote{...an experience of pure darkness is what it is by differing, in its particular way, from an immense number of other possible experiences...} \citep[p. 2]{oizumi2014phenomenology}

			
    			@ \todo{Need an example here} Follow the example in the last section, a small creatures may live in a simple environment and forms a NTIC process. The predictive information for the next state of the process may be very low. This also implies the richness of the conscious experience this agent has is very limited. 
    			
    			@ Our theory naturally explains why consciousness is compositional, because the joint states of entities in the NTIC process determine the contents of consciousness. \ideaCallout{think about it}
    			

				@ Importantly, note that because the closure is at a coarse-grained scale one state of the informational closure can be mapped to several states at microscopic scale.
				
				@ This supports multiple realisation thesis \needref{multiple realisation} which suggests same conscious experiences can be mapped to different physical implementations. 
	    ...
	    \subsubsection{Reconcile level and content of consciousness}
	        \begin{ants}
	            @ Our theory link conscious level and conscious content together via the state space of the NTIC process.
	            
	            @ When the information encapsulate in the NTIC process is large and reaches a high NTIC, based on our definition, the system has high level of consciousness. Simultaneously, this provides a rich state space which is equivalent to encoding rich predictive environmental dynamic in the system. 
	            
	            @ levels and contents of consciousness are just two different measures of a NTIC process. 
	            
	            @ this indicates an important prediction in which the conscious level is determined by states that the NTIC process potentially can reach rather the current state of a system. 
	            
	            @ For example, some neural states may map to simple sensory experience, e.g. visual experience in a dark room. However, this does not degrade the conscious level. This experience still provides a huge amount of information because it excludes enormous possibilities of visual experience and decrease a large amount of uncertainty.
...
\rlstart{1}
		\subsection{sensory hierarchy and neural coarse-graining}\label{sec:SensoryHierarchy}
			...
			\begin{ants}
				@ It's important to note that the coarse-graining direction is not necessary to be align with \critical{Just saying anatomical is not enough. Need better expression} anatomical sensory hierarchy in the neural system.

				@ the coarse-grained information is needed and computed only when the information is required for microscopic scale processing (readout). 
				
				@ Therefore, our theory is very different from consciousness theories focus on the anatomical hierarchy, for example the intermediate Level Theory \citep[see also \ref{IntermediateLevelTheory}]{prinz2007intermediate, jackendoff1987consciousness}.
				
				@ The truth is that due to the pervasive noise in the microscopic levels in the neural system reliable information processes need to be built upon the coarse-grained levels. 
				
				@ Nevertheless, to exercise causal power in actuators and environments, to read out information at coarse-grained levels is necessary. 
				
				@ In figure \ref{fig:hierarchy}, the blue arrows indicate the readout mechanisms implemented in the neural system. 
				
				@ We argue that the neural correlates of consciousness may not be the fundamental principles for consciousness. The data may contaminate the understanding of consciousness. 
			\end{ants}
...
\rlstart{1}
	\section{Conscious versus unconscious processing}
		...
		\begin{ants}
		    @ Our theory indicates that conscious information needs to fulfil two necessary conditions. 
		    
		    @ 1. The information is processed at a certain coarse-grained 
		    
		    @ 2. encoded in the NTIC process
		    
		    @ Based on the two conditions, we can classify brain processes into conscious and unconscious processing based on the two conditions. 
		...
		\subsection{Conscious Processing}
		    \begin{ants}
		        @ To achieve high NTIC has evolutionary advantage. 
		        
		        @ The current state of the process contains the information about its future state. Therefore, the neural system can build forward models to predict the future state of the environment using the information encoded in the NTIC process. 
		        
		        @ Cognitive functions recruiting forward models built upon it are conscious. (E.g., planning, imagination, dreaming)
		    \end{ants}
		
		\subsection{Unconscious Processing}
		    \begin{ants}
    		    @ When processes fail to satisfy either of the two conditions, they remain unconscious. 
    		    
    		    @ When processes do not meet the first condition, i.e. the information processed not at the certain coarse-grained level, the information will not be included in conscious contents. 
    		    
    		    @ For example, we are never conscious of neural spike activities at microscopic levels (e.g. single cell activities) because information passing among individual neurons are not at the certain coarse-grained level. 
    		    
    		    @ Libet's free will experiment can be also explained by our theory. 
    		    
    		    @ The random neural fluctuation under Libet's free will experiment happens at the microscopic scale \citep{SchurgerE2904}. 
    		    
    		    
    		    @ When processes do not meet the second condition, i.e. processes which do not achieve high NTIC will remain unconscious. 
    		    
    		    @ This can explain why many "policy-based" processes are not conscious. 
    		    
    		    @ Policy-based here features those processes taking sensory information as input and directly outputting actions. Therefore, the future states of policy-based processes largely depend on the environmental states and failed to achieve informational closure. 
    		    
    		    @ Several neural processes are included in this category, e.g. reflexive neural circuit, blindsight, procedure memory. 
		    \end{ants}
...
\rlstart{1}
		\subsection{Theories about multilevel views on consciousness and cognition}
			\begin{ants}
			    @ Previously, multilevel views on consciousness and cognition have been proposed. 
			    
			    @ In the following, we discussed the differences between our theory and other theories about multilevel views.
				
			\end{ants}
			
			\begin{writingMaterials}
			    @ LEVELS OF ORGANIZATION \cite{wimsatt1994ontology}
			    ...
			    @ See the discussion in \cite{pennartz2015brain}. But Pennartz focus more on multimodal representation as the most important factor.  
			    
			    @ Our proposal is inline with Pennartz's Neurorepresentational theory (Neurorepresentationalism). \cite{pennartz2018consciousness,pennartz2015brain}.
			    
			   
			    
			    
			    @ Some theories also address the relationship between level of processing and consciousness.
			    
			    @ However, it’s crucial to note that our theory claims only the level of coarsegraining achieving NTIC matters to connsciousness rather than the anatomical level of sensory hierarchy
			...
			\subsubsection{Intermediate Level Theory} \label{IntermediateLevelTheory}
				...
				@ As mentioned previously, a critical difference between ILT and our theory is that our theory doesn't assume the direction of coarse-graining should align with the direction of anatomical processing levels along cortical hierarchies (see fig \ref{fig:hierarchy}).
				\end{ants}
			...
			\subsubsection{Pennartz's neurorepresentational theory (Neurorepresentationalism)}
		        ...
		        @ However, Neurorepresentationalism focuses on multimodal representation.
		        
		        @ Neurorepresentationalism also focuses on "making sense" between representation
		        
		        @ We focus on informational closure which suggests the closure of causal interaction in a system. 
		        
		        @ We don't assume any functional properties as the criteria of consciousness. Instead, we propose information as a more fundamental principle correlates consciousness.  
			    \end{ants}
...
\rlstart{1}	
		\subsection{First-order theories}
		    ...
		    @ The ﬁrst-order theories suggest that consciousness is determined by early sensory information or neural activities.\needref{First-order theories}
		    
		    @ We suggest that, if early sensory information or neural activities after coarse-graining take parts in the NTIC process, early sensory information or neural activities also contribute consciousness. 
		    
		    @ However, due to coarse-graining, changes of early sensory information or neural activities do not necessarily accompany with changes of conscious contents. 
